...
def main(opt):  
    ...
    while True:
        ...
        if im is None:
            break        
        im0,pred_boxes,pred_confes,pred_classes = det.detect(im)    
        if len(pred_boxes) >0 :
            for i,_ in enumerate(pred_boxes):   
                box = pred_boxes[i]
                left,top,width,height = box[0], box[1], box[2], box[3]   
                box = (left,top,left+width, top+height)
                box = np.squeeze(scale_coords(shape, np.expand_dims(box,axis=0).astype("float"), im0.shape[:2]).round(),axis=0).astype("int")
                x0,y0,x1,y1 = box[0], box[1], box[2], box[3]
                cv2.rectangle(im, (x0, y0), (x1, y1), (0, 0, 255), thickness=2) 
                cv2.putText(im,'{0}--{1:.2f}'.format(pred_classes[i],pred_confes[i]),(x0, y0 - 10),cv2.FONT_HERSHEY_SIMPLEX,0.5, (0, 255, 0), thickness=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='weights/best.onnx', help='onnx path(s)')
